...
const LivreCtrl = {
  //Affichage de laliste complete des livres
  ...
  //Modifier un livre présent dans la base de données
  updateLivre: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ msg: "Livre mis à jour avec succès", updatedLivre });
    } catch (error) {
  ...
  //Supprimé un livre présent dans la base de données
