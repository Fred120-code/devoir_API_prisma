...
const userCtrl = {
  ...
  //connexion d'un utilsateur déjà enregistré dans la base de données
  login: async (req: Request, res: Response) => {
    try {
      const { email, password } = req.body;

      //verifie que tout les champs sont presents
      if (!email || !password) {
        res.status(400).json({ msg: "email et mot de passe requis" });
      }

      //verifie que l'utilisateur est present dans la base données
      ...
      if (!User) {
        ...
        //verifie que le mot de passe entré est correct
  ...
  //Modifications des informations d'un utilisateur presente dans la base de données
  updateUser: async (req: Request, res: Response) => {
    try {
      ...
      if (!existUser) {
        ...
        res
          .status(200)
          .json({ msg: "user mis a jour avec succes", user: updatedUser });
      }
    } catch (error) {
  ...
  //Supprimé un utilisateur présent dans la base de données
  deletUser: async (req: Request, res: Response) => {
    try {
      ...
      if (!user) {
        ...
        res
          .status(200)
          .json({ msg: "Compte de l'utilisateur supprimé avec succès", user });
      }
    } catch (error) {
